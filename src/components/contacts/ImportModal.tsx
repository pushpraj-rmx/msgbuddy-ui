  ...
  useEffect(() => {
    ...
    if (step === "analyzing") {
      ...
      if (
        ...
        setStep("result");
        if ((job.failedCount ?? 0) === 0 && job.status === "COMPLETED") {
          onSuccess();
        }
      }
    }
    ...
    <dialog open className="modal modal-middle">
      <div className="modal-box flex max-h-[85vh] max-w-2xl flex-col rounded-box border border-base-300 !bg-base-100 p-0">
        ...
        <div className="flex items-start justify-between gap-3 border-b border-base-300 px-5 py-4">
          <div>
            <span className="op-label">contacts · step {stepNumber} of 5</span>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        />
        <p className="text-[0.6875rem] text-base-content/50">
          ISO 3166-1 alpha-2. Used for phone numbers without a country code.
        </p>
      </div>
    ...
    <div className="space-y-4">
      <div className="rounded-box border border-base-300 bg-base-200 px-4 py-6 text-center">
        ...
        <p className="mt-1 text-[0.78125rem] text-base-content/55">
          Parsing rows, normalizing phones, and looking up existing contacts.
        </p>
        {job?.totalRows ? (
...
function PreviewStep({ job }: { job: ImportJob }) {
  const willCreate =
    (job.totalRows ?? 0) -
    (job.mergedDuplicates ?? 0) -
    (job.failedCount ?? 0); // approx; precise totals come from sample arrays
  void willCreate;
  // Use the granular counts from job sample arrays where available
  const sampleCounts = {
    create: job.sample?.create.length ?? 0,
    update: job.sample?.update.length ?? 0,
    skip: job.sample?.skip.length ?? 0,
    error: job.sample?.error.length ?? 0,
  };
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
        ...
        <Stat label="Create" value={sampleCounts.create} tone="success" />
        <Stat label="Update" value={sampleCounts.update} tone="info" />
        <Stat label="Skip" value={sampleCounts.skip} tone="muted" />
        <Stat label="Error" value={sampleCounts.error} tone="error" />
      </div>

      {(job.mergedDuplicates ?? 0) > 0 && (
        <div className="rounded-box border border-info/30 border-l-2 border-l-info bg-base-200 px-3 py-2.5">
          <span className="op-label mb-1 block text-info">duplicates merged</span>
          <p className="text-[0.8125rem] text-base-content">
            <span className="font-mono-op tabular-nums font-semibold">{(job.mergedDuplicates ?? 0).toLocaleString()}</span> rows in your file shared a phone with another row and were merged into a single contact (later non-empty fields win, tags union).
          </p>
        </div>
      )}
      ...
      {(job.newTags.length > 0 || job.newCustomFields.length > 0) && (
        <div className="space-y-3 rounded-box border border-warning/30 border-l-2 border-l-warning bg-base-200 px-3 py-2.5">
          <span className="op-label block text-warning">will be auto-created</span>
          {job.newTags.length > 0 && (
            <div>
              <span className="op-label mb-1 block">tags · {job.newTags.length}</span>
          ...
          {job.newCustomFields.length > 0 && (
            <div>
              <span className="op-label mb-1 block">
                custom field columns · {job.newCustomFields.length}
              </span>
              <div className="flex flex-wrap gap-1.5">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <Stat label="Created" value={job?.createdCount ?? 0} tone="success" />
        <Stat label="Updated" value={job?.updatedCount ?? 0} tone="info" />
        <Stat label="Skipped" value={job?.skippedCount ?? 0} tone="muted" />
        <Stat label="Failed" value={job?.failedCount ?? 0} tone={(job?.failedCount ?? 0) > 0 ? "error" : "muted"} />
      </div>

      <p className="text-[0.71875rem] text-base-content/55">
        You can leave this window open or close it — the import keeps running in the background. Use the contacts list once it finishes.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <Stat label="Failed" value={job.failedCount} tone={job.failedCount > 0 ? "error" : "muted"} />
      </div>

      {job.status === "CANCELLED" ? (
        <div className="rounded-box border border-warning/30 border-l-2 border-l-warning bg-base-200 px-3 py-2.5">
          <span className="op-label mb-1 block text-warning">cancelled</span>
          <p className="text-[0.8125rem] text-base-content">
      ...
      {job.status === "FAILED" && job.errorMessage ? (
        <div className="rounded-box border border-error/30 border-l-2 border-l-error bg-base-200 px-3 py-2.5">
          <span className="op-label mb-1 block text-error">failed</span>
          <p className="text-[0.8125rem] text-base-content">{job.errorMessage}</p>
        </div>
      ) : null}
      ...
      {job.errors.length > 0 && (
        <div className="overflow-x-auto rounded-box border border-base-300 bg-base-200">
          <div className="border-b border-base-300 px-3 py-2">
            <span className="op-label">errors · {job.errors.length}</span>
          </div>
          <table className="w-full text-[0.75rem]">
            <thead>
              <tr className="border-b border-base-300 bg-base-100">
                <th className="op-label px-3 py-2 text-left font-medium">Row</th>
                <th className="op-label px-3 py-2 text-left font-medium">Message</th>
              </tr>
            </thead>
            <tbody>
              {job.errors.slice(0, 50).map((e, i) => (
                <tr key={i} className="border-b border-base-300/50 last:border-b-0">
                  <td className="font-mono-op px-3 py-1.5 tabular-nums text-base-content/65">{e.row}</td>
...
function Stat({
  ...
  return (
    <div className="rounded-box border border-base-300 bg-base-200 px-3 py-2 text-center">
      <div className={`font-mono-op text-[1.25rem] font-semibold leading-none tabular-nums ${valueColor}`}>
  ...
  return (
    <div className="rounded-box border border-base-300 bg-base-200">
      <div className="flex items-baseline justify-between border-b border-base-300 px-3 py-2">
        <span className="text-[0.8125rem] font-semibold tracking-[-0.01em]">Samples</span>
        <span className="op-label">first 20 rows of each category</span>
      </div>
        ...
        <table className="w-full text-[0.75rem]">
          <thead>
            <tr className="border-b border-base-300 bg-base-100">
              ...
              <th className="op-label px-3 py-2 text-left font-medium">Phone</th>
              <th className="op-label px-3 py-2 text-left font-medium">Name</th>
              <th className="op-label px-3 py-2 text-left font-medium">Notes</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={`${r.action}-${r.row}-${i}`} className="border-b border-base-300/50 last:border-b-0">
                <td className="font-mono-op px-3 py-2 tabular-nums text-base-content/65">{r.row}</td>
                <td className="px-3 py-2"><ActionPill action={r.action} /></td>
                <td className="font-mono-op px-3 py-2 tabular-nums text-base-content/85">{r.phone}</td>
                <td className="px-3 py-2 text-base-content/70">{r.name ?? "—"}</td>
                <td className="px-3 py-2 text-[0.71875rem]">
                  {r.action === "update" && r.diff ? (
                    <DiffSummary diff={r.diff} />
                  ) : r.reason ? (
                    <span className="text-base-content/55">{r.reason}</span>
                  ) : (
    ...
    <div className="space-y-0.5">
      {entries.map(([field, d]) => (
        <div key={field} className="leading-tight">
          ...
          <span className="text-base-content/40 line-through">{d.from ?? "—"}</span>
